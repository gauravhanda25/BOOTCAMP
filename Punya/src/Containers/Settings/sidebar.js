...
const renderSidebar = () => {
    ...
    if (userData.user.account_id != config.JUVLY_ACC_ID && userData.user.account_id != config.CC_ACC_ID) {
      ...
      if (userData.account.pos_gateway === 'stripe' && userData.account.account_type === 'paid' && userData.account.stripe_mode === 'custom' && userData.user_type === 'superadmin') {
          ...
          if(getIsPosEnabled()){
            ...
            {
                label: languageData.settings['sidebar_POS_disputes_only_menu'],
                to: '/settings/pos-disputes',
                match: ['/settings/pos-disputes']
            }
          );
    ...
    sideBarContent.push({
        label: languageData.settings['sidebar_dot_phrase'],
        to: '/settings/dot-phrases',
        match: ['/settings/dot-phrases']
    })
...
class Sidebar extends React.Component {
    ...
    manageSideBar = (selected_parent_id,grandParentNode,element) => {
        let el;
        if(!grandParentNode && element !== undefined){
            el = element;
        }else{
            el = grandParentNode.getElementsByClassName('parent_class');
        }
        _.forOwn(el, (value, key) => {
            if(selected_parent_id != key){
                value.setAttribute('class','parent_class new-setting-subtabs-a');
                if(value.nextSibling){
                    value.nextSibling.setAttribute('class','setting-submenu no-display');
                    value.querySelector('svg').setAttribute('class','svg-inline--fa fa-angle-right fa-w-8');
                }
            }
        });
    }
    handleClickBar = (selected_parent_id) => {
       this.manageSideBar(selected_parent_id, null , document.getElementsByClassName("parent_class new-setting-subtabs-a"));
    }
    toggleNav = (e) => {
        let selected_parent_id = e.target.getAttribute('parent_id');
        ...
        this.manageSideBar(selected_parent_id,grandParentNode,null);
    }
      ...
      if (obj !== undefined) {
          obj.map((io, ix) => {
            if ( io.match && io.match !== undefined) {
              if ( this.props.match.path.slice(9).startsWith(io.to.slice(9)) > -1 && this.props.match.url.startsWith(io.to) ) {
                if ( io.content ) {
    ...
    render() {
        ...
        nav = this.state.sideBarContent.map((obj, idx) => {
            ...
            return (<li className={parentLiClass} key={idx} ref={'parents'} >
                {(obj.to !== undefined) ? <Link id={idx} className={toggleClasss} onClick={() =>  this.handleClickBar(idx)} to={(obj.to !== undefined) ? obj.to : "javascript:void(0)"}> {obj.label} </Link> : <a parent_id={idx} className={toggleClasss} onClick={obj.content !== undefined ? this.toggleNav.bind(this) : undefined} href={(obj.to !== undefined) ? obj.to : "javascript:void(0)"}> {obj.label}
                ...
                {obj.content !== undefined && obj.content.length > 0 &&
                    <StyleRoot className={subMenuULClass}>
                        <ul className={"setting-submenu"} style={styles.fadeIn}>
...
//export default connect({}, mapDispatchToProps)(Pos);
