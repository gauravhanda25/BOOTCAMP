...
import { getUser,userProfile, uploadImage,getPreTreatmentEmail,preEmailUpdate } from '../../../../Actions/Settings/settingsActions.js';
import {SketchField, Tools} from 'react-sketch';


class PreTreatmentEmail extends Component{
  constructor(props){
    super(props);
    const userData = JSON.parse(localStorage.getItem('userData'));

    this.state={
      pre_treatment_subject:'',
      pre_treatment_body:'',
      userChanged:false,
      emailList: {},
  ...
  componentDidMount(){
    ...
    this.setState({
      ...
      pre_treatment_email_header:languageData.settings['pre_treatment_email_header'],
      clinic_Please_Wait: languageData.settings['clinic_Please_Wait'],
    })
    this.setState({'showLoader': true});
    this.props.getPreTreatmentEmail();
  }
    ...
    if(props.showLoader != undefined && props.showLoader == false) {
      ...
      if (props.emailList !== undefined && props.emailList != state.emailList) {
          return {
              ...
              pre_treatment_subject: (state.userChanged) ? state.pre_treatment_subject : props.emailList.pre_treatment_subject,
              pre_treatment_body: (state.userChanged) ? state.pre_treatment_body : props.emailList.pre_treatment_body,
              showLoader : false,
              //isDisabled: true
          };
      }  else
  ...
  handleSubmit =(event) => {
    ...
    if (this.state.pre_treatment_subject === '' || this.state.pre_treatment_subject.trim() == "") {
      //toast.error("Pre treatment subject cannot be blank");
      this.setState({
        isDisabled: false,
        subjectError: true
      })
    ...
    if (this.state.pre_treatment_body === '' || this.state.pre_treatment_body.trim() == "") {
      //toast.error("Pre treatment body cannot be blank");
      this.setState({
        isDisabled: false,
        bodyError: true
      })
    ...
    let formData = {
        pre_treatment_subject: this.state.pre_treatment_subject,
        pre_treatment_body: this.state.pre_treatment_body,
    }
    this.props.preEmailUpdate(formData);
  }
        ...
        <Sidebar />
          <div className="setting-setion">
            ...
            <div className="setting-container">
              <div className="setting-title">{this.state.pre_treatment_email_header}
              </div>
              <div className="row">
                  ...
                  <div className="row">
                    <div className="col-sm-12">
                      ...
                      <div className="setting-field-outer">
                        ...
                        <input name="pre_treatment_subject" id="pre_treatment_subject" className={this.state.subjectError === true ? "setting-input-box setting-input-box-invalid" :"setting-input-box"} placeholder="Subject" value={this.state.pre_treatment_subject} disabled={this.state.isDisabled} autoComplete="off" onChange={this.handleInputChange} type="text" />
                      </div>
                    </div>
                    <div className="col-sm-12">
                      <div className="setting-field-outer">
                        <div className="new-field-label">{this.state.post_treatment_email_body}<span className="setting-require">*</span></div>
                        <textarea name="pre_treatment_body" id="pre_treatment_body" className={this.state.bodyError === true ? "setting-textarea-box m-h-300 setting-input-box-invalid" :"setting-textarea-box m-h-300"} placeholder="Email body" disabled={this.state.isDisabled} onChange={this.handleInputChange} value={this.state.pre_treatment_body} />
                      </div>
                    </div>
                  </div>
...
function mapStateToProps(state) {
    const languageData = JSON.parse(localStorage.getItem('languageData'));
    const returnState = {};
    if (state.SettingReducer.action === "PRE_TREATMENT_EMAIL_LIST") {
      if(state.SettingReducer.data.status != 200) {
        toast.error(languageData.global[state.SettingReducer.data.message]);
        returnState.showLoader = false
      } else {
        returnState.emailList = state.SettingReducer.data.data;
      }
    } else if (state.SettingReducer.action === "PRE_TREATMENT_EMAIL_UPDATE") {
      if(state.SettingReducer.data.status != 200){
        toast.error(languageData.global[state.SettingReducer.data.message]);
        returnState.showLoader = false
      }
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({
      getPreTreatmentEmail: getPreTreatmentEmail,
      preEmailUpdate:preEmailUpdate
    }, dispatch)

}

export default connect(mapStateToProps, mapDispatchToProps)(PreTreatmentEmail);
